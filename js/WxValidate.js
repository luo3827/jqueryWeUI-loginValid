...
class WxValidate {
  ...
  __initDefaults () {
    this.defaults = {
      messages: {
        ...
        max: this.formatTpl('请输入大于 {0} 的数值。'),
  ...
  __initMethods () {
    ...
    that.methods = {
       ...
       */
      unifiedCode (value) {
        return that.optional(value) || (value.length == 2)
      },
      /**
       * 验证码
       */
      securityCode (value) {
        return that.optional(value) || (value.length == 4)
      },
      /**
       * 验证两个输入框的内容是否相同
       */
  ...
  * 测试onblur事件
  * */
  checkBlurForm (data, param) {
    this.__initData()

    for (let item of param) {
        this.setView(item)
        this.checkParam(item, this.rules[item], data)
    }
    // this.setView(param)
    // this.checkParam(param, this.rules[param], data)
